...
download_package() {
    local pkg_name="$1"
    local packages_dir="${REPO_URL}/Packages"
    local first_letter="${pkg_name:0:1}"
    local search_url="${packages_dir}/"

    if curl -L -s -f -I -A "$USER_AGENT" "${packages_dir}/${first_letter}/" >/dev/null 2>&1; then
        search_url="${packages_dir}/${first_letter}/"
        echo "[$pkg_name] Using subdirectory: ${first_letter}/" >&2
    fi

    echo "[$pkg_name] Fetching package list from: ${search_url}" >&2
    local html_page=$(mktemp)

    if ! curl -L -s -f -A "$USER_AGENT" --max-time 90 --retry 2 -o "$html_page" "${search_url}"; then
        echo "[$pkg_name] ERROR: Failed to fetch package listing" >&2
        rm -f "$html_page"
        return 1
    fi

    local html_size=$(wc -c < "$html_page" 2>/dev/null || echo "0")
    echo "[$pkg_name] Downloaded ${html_size} bytes" >&2

    if [ "$html_size" -eq 0 ]; then
        echo "[$pkg_name] ERROR: Empty response from server" >&2
        rm -f "$html_page"
        return 1
    fi
...
for lib_pattern in "libbfd*.so*" "libopcodes*.so*" "libctf*.so*" "libmpc.so*" "libgmp.so*" "libmpfr.so*"; do
    for lib in usr/lib64/${lib_pattern}; do
        [ -f "$lib" ] && ln -sf "../$(basename "$lib")" usr/lib64/toolchain/
    done
done
shopt -u nullglob
